...
func main() {
	logger.Init(&logger.Option{
		ServiceName:    env.GetEnvWithDefaultAsString("SHIPPER_SERVICE", "Shipper"),
		ServiceVersion: env.GetEnvWithDefaultAsString("SHIPPER_VERSION", "0.0.1"),
	...
	c := consumers.Consumer{
		...
		Topic:  topics.TopicOrderPickedAndPacked,
	}

	log.Fatal(c.ShipperSubscribeAndListen())
}
